...
`README.md` as the design document / target architecture, not a
...
and the inventory NetHub renders around it (README §3.5/§8.1); its
...
That manual, interactive form is what is committed today. README §8.1
...
**Day-2 is two separate EE dispatches, not one** (README §6, §8/§8.1).
...
**Everything the EE reads is rendered, not supplied** (README §3.5).
...
(README §4.3). The admin session is OIDC keyed on the `sub` claim, checked
...
NetHub is explicitly not an inventory system (README §2 Non-goals, §7.4).
...
These are settled decisions with reasoning in the README. If a change
...
- **No user-supplied playbooks.** The playbook set is closed at build
  time and selected by `platform` (README §2 Non-goals, §8.1). Accepting
...
- **No EE invocation from the Flask process.** Flask holds the only
  ...
  host-level container control (README §9). The sibling owns dispatch and
...
Describes the playbook as committed. README §8.1 retires several of these.
...
This file is derived from `README.md` plus the actual state of the repo,
